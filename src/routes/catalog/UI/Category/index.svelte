<script>
	...
	$inspect(isVisibleCurrentProduct);
	// $inspect('data', data);
</script>
		...
		<tbody data-accordion="table-column">
			{#each data.product as item}
				...
				>
					...
					<td class="w-4 p-3">
                        <button onclick={() => (isVisibleCurrentProduct.value = !isVisibleCurrentProduct.value)} type="button" aria-label="Раскрыть строку" class="cursor-pointer">
                            <svg
							data-accordion-icon=""
					...
					<td class="whitespace-nowrap px-4 py-3">
						<div
							class="mr-2 w-fit rounded bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800"
						>
							Active
						</div>
					</td>
				...
				>
					<td class="border-b p-4" colspan="9">
						...
						<div class="mt-4 grid grid-cols-4 gap-4">
							<div
								class="relative flex flex-col items-start justify-between rounded-lg bg-gray-100 p-3"
							>
